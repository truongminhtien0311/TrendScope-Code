...
- [ ] Tải ảnh về máy (`src/lib/storage/`) để không phụ thuộc link Trung Quốc
- [ ] Đăng nhập (gợi ý: thư viện **Auth.js** — bảng User đã chuẩn bị sẵn)
- [ ] Chia sẻ cho team: **Cloudflare Tunnel** (miễn phí, trỏ link vào máy mình — chính là "link đăng nhập cloudflare" trong mindmap) hoặc deploy lên VPS
- [ ] Nếu nhiều người dùng đồng thời: đổi SQLite → PostgreSQL (sửa `prisma/schema.prisma` + connection string)
- [ ] Đồng bộ Google Drive / Lark
