...
- [x] Đăng nhập nhiều tài khoản qua **Google OAuth** (`src/app/api/auth/google/`) — session giữ bằng `iron-session` (tự viết, không dùng Auth.js), xem `src/lib/auth.ts`, `src/middleware.ts`. Bảng User có `role` admin/member, quản lý tài khoản ở Cài đặt > Bảo mật (`src/components/SecurityPanel.tsx`). *(Trước đây dùng đăng nhập mật khẩu tự viết bằng `bcryptjs` — đã bỏ hẳn, xoá `login/route.ts`/`change-password/route.ts`, không còn field `passwordHash` trong schema.)*
